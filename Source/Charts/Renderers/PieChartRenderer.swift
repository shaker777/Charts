...
open class PieChartRenderer: DataRenderer
{
    ...
    func getDataSetPaths(dataSet: IPieChartDataSet, varRadius: Bool = true) -> [CGMutablePath] {
        guard let chart = chart else {return []}
        ...
        var drawRadii = chart.drawRadii
        let center = chart.centerCircleBox
        let totalRadius = chart.radius
        let drawInnerArc = chart.drawHoleEnabled && !chart.drawSlicesUnderHoleEnabled
        let userInnerRadius = drawInnerArc ? totalRadius * chart.holeRadiusPercent : 0.0
...
//                {
                    ...
                    let sliceSpaceAngleOuter = visibleAngleCount == 1 ?
                        0.0 :
                        sliceSpace / sliceRadius.DEG2RAD
                    ...
                    let arcStartPointX = center.x + sliceRadius * cos(startAngleOuter.DEG2RAD)
                    let arcStartPointY = center.y + sliceRadius * sin(startAngleOuter.DEG2RAD)
                    
                    
                    path.move(to: CGPoint(x: arcStartPointX,
                                          y: arcStartPointY))
                    
                    path.addRelativeArc(center: center, radius: sliceRadius, startAngle: startAngleOuter.DEG2RAD, delta: sweepAngleOuter.DEG2RAD)
                        ...
                        (innerRadius > 0.0 || accountForSliceSpacing)
                    {
                        if accountForSliceSpacing
                        {
                            ...
                            innerRadius = min(max(innerRadius, minSpacedRadius), sliceRadius)
                        }
...
//        let dataScale =
        ...
        let shadowPath = paths.reduce(CGMutablePath()) { (result, val) -> CGMutablePath in
            result.addPath(val)
            return result
        }
        ...
        let shadowBlurRadius: CGFloat = 6
        context.setShadow(offset: shadowOffset, blur: shadowBlurRadius, color: shadow.cgColor)
        
        
        let cp = shadowPath.copy(strokingWithWidth: 1, lineCap: .butt, lineJoin: .round, miterLimit: 0);
        context.addPath(cp)
        ...
        if chart.drawTotalCircle == true{
            let dashPath = UIBezierPath();
            dashPath.addArc(withCenter: center, radius: radius, startAngle: CGFloat(0).DEG2RAD, endAngle: CGFloat(360).DEG2RAD, clockwise: true)
            
            context.saveGState()
            context.addPath(dashPath.cgPath)
            context.setStrokeColor(chart.totalCircleColor.cgColor)
            context.setLineWidth(3);
            context.setLineCap(.round)
            context.setLineJoin(.round)
            let dashes: [CGFloat] = [0.0, 11.2]
            context.setLineDash(phase: 0, lengths: dashes)
            context.drawPath(using: .stroke)
            context.restoreGState();
            
            let attributes = [NSAttributedStringKey.font: chart.totalCircleFont,
             NSAttributedStringKey.foregroundColor: chart.totalCircleColor]
            let stringSize = NSString.init(string: "100%").size(withAttributes: attributes)
            ChartUtils.drawText(
                context: context,
                text: "100%",
                point: CGPoint(x: center.x, y: center.y - radius - stringSize.height - 2),
                align: .center,
    ...
    open override func drawValues(context: CGContext)
    {
        ...
        let totalRadius = chart.radius
        let sliceMaxRadius = chart.maxRadius
        let rotationAngle = chart.rotationAngle
        var drawAngles = chart.drawAngles
        var drawRadii = chart.drawRadii
        ...
        let xValueSum = (data as! PieChartData).xValueSum
        ...
        for i in 0 ..< dataSets.count{
            ...
            let paths = getDataSetPaths(dataSet: dataSet);
            let fullPath = getDataSetPaths(dataSet: dataSet, varRadius: false)
            ...
            for j in 0 ..< dataSet.entryCount{
                ...
                let value = usePercentValuesEnabled ? e.x / xValueSum * 100.0 : e.x
                ...
                let valueTextSize = NSString(string: valueText).size(withAttributes: [NSAttributedStringKey.font : valueFont])
...
//                                      (valueTextSize.height-valueTextCenter.y)/sliceYBase]
                ...
                let holeRaduis = totalRadius*chart.transparentCircleRadiusPercent;
                let minimumRadius = max(holeRaduis + valueTextRadius, (sliceRadius - holeRaduis) / 2 + holeRaduis );

                var valueDrawRadius: CGFloat!
                var valueDrawPoint: CGPoint!
                
                //from min to slice radius
                var expectedRadius = minimumRadius
                while expectedRadius < (sliceRadius - valueTextRadius - edgeOffset) && valueDrawRadius == nil{
                    
                    let drawRaduis = CGFloat(expectedRadius)
                    let drawPoint = CGPoint(x: drawRaduis * sliceXBase + center.x, y: drawRaduis * sliceYBase + center.y - valueTextCenter.y)
                    
                    if  slicePath.contains(CGPoint(x: drawPoint.x - valueTextSize.width/2, y: drawPoint.y)) &&
                        slicePath.contains(CGPoint(x: drawPoint.x + valueTextSize.width/2, y: drawPoint.y)){
                    
                        valueDrawRadius = drawRaduis
                        valueDrawPoint = drawPoint
                        break;
                    }
                    expectedRadius += 1;
                }
                
                //from slice radis till total
                expectedRadius = max(minimumRadius + edgeOffset, sliceRadius + valueTextRadius + edgeOffset)
                while expectedRadius < totalRadius && valueDrawRadius == nil{

                    let drawRaduis = CGFloat(expectedRadius)
                    let drawPoint = CGPoint(x: drawRaduis * sliceXBase + center.x, y: drawRaduis * sliceYBase + center.y - valueTextCenter.y)
                    
                    if fullSlicePath.contains(CGPoint(x: drawPoint.x - valueTextSize.width/2, y: drawPoint.y)) &&
                        fullSlicePath.contains(CGPoint(x: drawPoint.x + valueTextSize.width/2, y: drawPoint.y)){
...
//                        print("")
                ...
                //draw line
                if dataSet.valueLineColor != nil && (valueDrawRadius - sliceRadius) > 20 {
                    ...
                    let jointPadding: CGFloat = 5
                    if sliceXBase < 0{
                        context.move(to: CGPoint(x: valueDrawPoint.x-valueTextSize.width/2 , y: valueDrawPoint.y + valueTextSize.height + bottomPadding))
                        context.addLine(to: CGPoint(x: valueDrawPoint.x+valueTextSize.width/2 + jointPadding, y: valueDrawPoint.y + valueTextSize.height + bottomPadding))
                    }else{
                        context.move(to: CGPoint(x: valueDrawPoint.x+valueTextSize.width/2, y: valueDrawPoint.y + valueTextSize.height + bottomPadding))
                        context.addLine(to: CGPoint(x: valueDrawPoint.x-valueTextSize.width/2 - jointPadding, y: valueDrawPoint.y + valueTextSize.height + bottomPadding))
                    }
...
//                        height: holeRadius * 2.0))
            ...
            context.saveGState()
            let cp = path.copy(strokingWithWidth: 2, lineCap: .butt, lineJoin: .round, miterLimit: 0);
            context.addPath(cp)
            context.setStrokeColor(UIColor.white.cgColor)
